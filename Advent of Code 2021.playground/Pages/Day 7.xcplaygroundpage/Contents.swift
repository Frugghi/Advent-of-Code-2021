...
let input = try Input.day7.load(as: [Int].self, separators: [",", "\n"])

var minimum = input[0]
var maximum = input[0]
var counters: [Int: UInt] = [:]
for input in input {
...
struct PartTwoCrabSubmarineEngine: CrabSubmarineEngine {
    private(set) var fuelConsumptionRate: UInt = 0
    private var crabsCount: UInt = 0

    mutating func adjustFuelConsumptionRate(_ newCrabs: UInt) {
        crabsCount += newCrabs
        fuelConsumptionRate += crabsCount
    }
}
