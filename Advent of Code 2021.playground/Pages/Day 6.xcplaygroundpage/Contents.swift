...
let countLanternfish = { (input: [UInt], numberOfDays: Int) -> UInt in
    ...
    for input in input {
        counters[Int(input)] += 1
    }

    while numberOfDays > 0 {
        let toBeSpawn = counters[0]
        counters[0] = 0

        let next = min(numberOfDays, zip(counters.indices, counters).first(where: { $0.1 > 0 }).map(\.0)!)
        numberOfDays -= next

        for index in next..<counters.endIndex {
            counters[index - next] = counters[index]
            counters[index] = 0
        }

        counters[6 - (next - 1)] += toBeSpawn
        counters[8 - (next - 1)] += toBeSpawn
    }
